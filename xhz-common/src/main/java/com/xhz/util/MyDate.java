...
public class MyDate {
	...
	public String yearMonthDayFormat = "yyyy年MM月dd日";
	...
	/**
	 * 获得当前时间,按格式为yyMMdd的字符串类型返回
	 * @return 返回字符串类型时间，如：20140101
	 */
	public String getCurrentNoConnectDate() {
	...
	/**
	 * 将指定时间，按世界标准时间以 yyyy年MM月dd日格式返回字符串类型的时间
	 * @param date 指定时间
	 * @return 返回字符串类型的时间
	 */
	public String toDateString(Date date) {
		...
		SimpleDateFormat df = new SimpleDateFormat(yearMonthDayFormat);
		return df.format(date);
	}
	/**
    ...
    /**
     * 计算日期格式为 2018-02-10 的两个字符串日期相差天数的绝对值
     * @param dateTime1
     * @param dateTime2
     * @return
     */
    ...
    /**
     * 计算日期格式为 2018-02-10 的两个字符串日期相差天数
     * @param dateTime1
     * @param dateTime2
     * @return
     */
    public Integer dateDayTimeDifferNoAbs(String dateTime1, String dateTime2){
    ...
    /**
     * 将指定的Date类型的日期以yyyy年MM月dd日格式返回字符串类型的时间
     * @param date
     * @return
     */
    public String geChDateString(Date date){
    	SimpleDateFormat df = new SimpleDateFormat(yearMonthDayFormat);
		return df.format(date);
    }
   ...
   */
	 ...
	 */
     ...
     /**
      * 将中文日期转换数据库日期格式,例如：传入"2018年01月01日",转换成"2018-01-01"
      * @param chDate
      * @return
      */
     public String chDate2dbDate(String chDate) {
    	if (null == chDate) {
    		return null;
    	}
    	SimpleDateFormat dfFrom = new SimpleDateFormat(yearMonthDayFormat);
 		SimpleDateFormat dfTo = new SimpleDateFormat(dateFormat);
 		String dbDate = null;
 		try {
 			Date date = dfFrom.parse(chDate);
 			dbDate = dfTo.format(date);
 		} catch (ParseException e) {
 			e.printStackTrace();
 		}
 		return dbDate;
     ...
     /**
      * 将数据库日期转换中文日期格式,例如：传入"2018-01-01",转换成"2018年01月01日"
      * @param dbDate
      * @return
      */
     public String dbDate2chDate(String dbDate) {
    	if (null == dbDate) {
    		return null;
    	}
    	SimpleDateFormat dfFrom = new SimpleDateFormat(dateFormat);
 		SimpleDateFormat dfTo = new SimpleDateFormat(yearMonthDayFormat);
 		String chDate = null;
 		try {
 			Date date = dfFrom.parse(dbDate);
 			chDate = dfTo.format(date);
 		} catch (ParseException e) {
 			e.printStackTrace();
 		}
 		return chDate;
     ...
     /**
      * 将中文日期格式转换为date类型,例如：传入"2018年01月01日",转换对应的date。
      * @param dateString
      * @return
      */
     public static Date parseChDate(String dateString) {
     ...
     /**
      * 将date类型,转为"2018年01月01日"字符串
      * @param date
      * @return
      */
     public String formatChDate(Date date) {
    	 if(date == null) {
    		 return null;
    	 }
    	 SimpleDateFormat sdf = new SimpleDateFormat("yearMonthDayFormat");
    	 try {
    		 String chDate =  sdf.format(date);
    		 return chDate;
		} catch (Exception e) {
			return null;
		}
     ...
     /**
      * 当日期格式为2018/03/23或2018-02-07时，转换成yyyy年MM月dd日格式
      * @param reportDate
      * @return
      */
	public String formatReportDate(String reportDate) {
		...
		if (reportDate.contains("/")) {
			...
			try {
				Date date = dfFrom.parse(reportDate);
				reportDate = dfTo.format(date);
			} catch (ParseException e) {
				e.printStackTrace();
			}
	...
	public static String getLocalCurrentDateTime() {
     ...
     public static void main(String[] args) {
    	...
    	String date2 = "2018年02月07日";
		System.out.println(new MyDate().formatReportDate(date1));
		System.out.println(new MyDate().formatReportDate(date2));

		Integer diff = new MyDate().dateDayTimeDifferNoAbs("2018-04-23", "2018-04-24");
		System.out.println(diff);
		System.out.println(parseChDate(date2));
